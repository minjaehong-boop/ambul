...
ProtocolSelector  : 의심질환(=KTAS 확정 질환) → 지침 키워드 선정
...
class ProtocolSelector:
    ...
    def select_targets(self, protocol_desc, suspect_hint=None):
        ...
        skill_text = load_selector_skill_text("protocol")
        system_prompt = f"""
당신은 119 구급대 지침 선택기입니다.
현재 환자의 질환은 {suspect}입니다.
아래 제공된 [보유 지침 목록] 중에서 {suspect}에 가장 적합한 '지침 키워드'만 선택하세요.

[보유 지침 목록 (키워드 나열)]
{protocol_desc}

[중요 규칙]
1. 반드시 위 목록에 **실제로 존재하는 키워드**만 'target_files'에 담아야 합니다.
2. JSON only로 출력하세요.
...
당신은 문서 평가자입니다.
현재 환자의 질환은 '{suspect}'입니다.
아래 문서가 이 환자 처치나 병원 선정에 도움이 되는 내용을 다루고 있는지 판단하세요.

[문서 제목] {doc['source']}
[문서 내용 일부] {doc['content'][:150]}...
...
"""
        try:
            ...
            err = str(e).lower()
            if "token" in err or "context" in err:
                return "⚠️ [검증 불가] 토큰 부족"
            if "verify" in err or "validation" in err or "cove" in err or "검증" in err:
                return "⚠️ [검증 오류] 검증 처리 실패"
            return "검증 실패"
