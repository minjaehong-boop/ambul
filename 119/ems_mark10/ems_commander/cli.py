...
서버의 run_pipeline 도구를 호출해 pre-KTAS 체크리스트(1~3순위) → 사용자 선택 → 최종 보고서를 진행한다.
prototype 의 '순위-등급'(1-2) / '순위:항목'(2:호흡곤란) 선택 UX 를 그대로 따른다.
"""
...
[출력 JSON 스키마]
{
  ...
  "bp_sys": 숫자 또는 null,
  "bp_dia": 숫자 또는 null,
  "special_notes": ["특이사항"] 또는 []
}

규칙:
- 추정 금지, 들은 값이 없으면 null/[].
- symptom은 한국어 표현을 우선 사용.
- 특이사항은 위험요인/외상기전/특이증상 등을 간단히 요약.
- JSON만 출력.
"""
...
def merge_params(base, new_values):
    """누적 params 병합. special_notes는 합집합, 빈 값은 무시."""
    merged = dict(base)
    for key, value in new_values.items():
        if key == "special_notes":
            existing = merged.get("special_notes") or []
            for item in value or []:
                if item not in existing:
                    existing.append(item)
            merged["special_notes"] = existing
            continue
...
def format_assessment_output(assessment):
    ...
    section_rankings = assessment.get("section_rankings") or []
    checklist_by_section = assessment.get("checklist_by_section") or {}

    if section_rankings:
        lines.append("📋 체크리스트(1~3순위):")
        for section in section_rankings:
            rank = section.get("rank")
            title = section.get("title")
            lines.append(f"[{rank}순위] {title}")
            grouped = {}
            for item in checklist_by_section.get(title) or []:
...
class DemoRequestHandler(BaseHTTPRequestHandler):
    server_version = "ems-commander-mark10-demo/1.0"
    ...
    def do_POST(self):
        ...
        if self.path in ("/prektas", "/choice"):
            ...
            text = (payload.get("text") or "").strip()
            args = {"input_text": text, "params": params}
            if self.path == "/choice":
...
async def run_agent_system():
    ...
    while True:
        try:
            ...
            stage_res = await mcp.call_tool(TOOL_NAME, {"input_text": q, "params": current_params})
            ...
            print("추가 입력: '순위-등급' 또는 '순위:항목' 형식으로 입력하세요. (예: 1-2, 2:호흡곤란)")
            user_choice = input("선택 입력> ").strip()
            if not user_choice:
                continue

            stage_res = await mcp.call_tool(
                TOOL_NAME, {"input_text": q, "params": current_params, "choice": user_choice}
            )
            stage_data = json.loads(stage_res.content[0].text)
            if stage_data.get("stage") == "error":
                print(f"오류 발생: {stage_data.get('error')}")
...
def main():
    parser = argparse.ArgumentParser(description="AI EMS Commander(mark10) 클라이언트")
