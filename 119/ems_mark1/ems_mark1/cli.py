"""mark1 클라이언트 — Parser LLM이 의심질환/검색질의/진료과를 뽑아 파이프라인 호출."""
...
[필수 필드]
1. suspect: 의심 질환명
2. knowledge_query: 프로토콜 검색용 질문
3. hospital_dept: 필요한 진료과
    ...
    while True:
        try:
            ...
            if not q:
                continue
            logger.log("user", q)
            print("[Commander] 분석 중...")
            plan = plan_mission(client, q, cmd_model)
            if not plan:
                print("상황 분석 실패")
                continue
            logger.log("plan", str(plan))
            print(f"[Plan] {plan.get('suspect')} (과: {plan.get('hospital_dept')})")
            print("[System] 파이프라인 실행...")
