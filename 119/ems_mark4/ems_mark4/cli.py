"""mark4 클라이언트 — 카테고리 desc 기반 target_files 선정 (진료과 없음)."""
...
def plan_mission(client, user_input, model, protocol_desc_str):
    ...
    [중요 규칙]
    'target_files'에는 카테고리 영어 이름을 쓰지 마십시오.
    반드시 위 'desc'에 적혀 있는 **단어 그대로**를 발췌하십시오.
    (예시: "심정지", "응급 분만", "경련", "화상")

    [임무]
    1. 환자의 증상을 분석하여 '의심 질환명(suspect)'을 도출하세요.
    2. 'desc' 내용을 참고하여 실제 파일명과 매칭될 **한글 키워드**를 'target_files'에 담으세요.
