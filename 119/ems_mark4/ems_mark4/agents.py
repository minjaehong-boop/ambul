"""mark4 파이프라인 LLM 에이전트 — CRAG / Reader(병원 DB 없이 지침 내 이송기준) / Expert."""
...
class ReaderAgent:
    """병원 DB 없이, 이송 병원 '기준'을 지침 본문에서 도출."""
    ...
    def synthesize_report(self, suspect, protocols):
        ...
        3. **이송 병원 선정 기준**:
           - 지침 내에 '병원 선정', '이송 기준', '의료기관 선정' 관련 내용이 있다면 이를 바탕으로 작성.
           - 예: "권역응급의료센터로 이송", "재가온 치료가 가능한 병원으로 이송" 등.
           - 특정 병원명을 지어내지 말고, **병원의 유형/조건**을 명시하십시오.
        """
        try:
            res = self.client.chat.completions.create(
...
class ExpertAgent:
    ...
    def validate_with_cove(self, draft_report, suspect, original_protocols):
        ...
        Especially check if the 'Transport Decision' (이송 병원 기준) matches the protocol.
