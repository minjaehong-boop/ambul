"""mark3 MCP 서버 — Direct Access 파이프라인.

execute_ems_pipeline(suspect, target_files, hospital_dept):
    파일 직접 로드 → 병원 조회 → CRAG → Reader → CoVe
"""
...
mcp = FastMCP("ems-commander-mark3")
...
@mcp.tool()
def execute_ems_pipeline(suspect: str, target_files: list[str], hospital_dept: str) -> str:
    """Args:
        suspect: 의심 질환명
...
def main():
    print(f"🚀 EMS Commander(mark3) MCP 서버 시작 (http://{config.MCP_HOST}:{config.MCP_PORT})")
