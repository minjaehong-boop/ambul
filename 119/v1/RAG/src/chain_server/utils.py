...
def get_embedding_model() -> Embeddings:
    ...
    if settings.embeddings.model_engine == "huggingface":
        ...
        return OpenAIEmbeddings(
            ...
            openai_api_base=settings.embeddings.server_url,
            openai_api_key="none",
        )
    else:
        raise RuntimeError(
            "Unable to find any supported embedding model. Supported engine is huggingface, nvidia-ai-endpoints, and openai."
        )
...
def get_ranking_model() -> BaseDocumentCompressor:
    ...
    try:
        if settings.ranking.model_engine == "nvidia-ai-endpoints":
            ...
            logger.info(f"Ranking with openai engine not yet supported locally; skipping reranker.")
            return None
        else:
